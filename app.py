...
def log_question_and_rating_to_s3(question=None, rating=None, uploaded_file_name=None, question_id=None):
    """
    Efficiently logs questions and ratings to S3 with proper encoding for Georgian characters.
    Only saves to S3 when a rating is provided.
    """
    try:
        # Create S3 client and set up constants
        timestamp = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        ...
        # For a new question, just generate and return a question_id without saving to S3
        if question_id is None and question is not None and rating is None:
            # Only generate question_id for tracking without saving
            return str(uuid.uuid4())
            
        # Handle new question with rating (full log entry)
        elif question_id is None and question is not None and rating is not None:
            # Create a new complete entry with question and rating
            question_id = str(uuid.uuid4())
            new_row = {
                "timestamp": timestamp,
                "file_name": uploaded_file_name or 'None',
        ...
        # Handle rating update for existing question_id
        elif question_id is not None and rating is not None:
            # Check if this question already exists in the log
            if "question_id" in df.columns and df["question_id"].astype(str).eq(str(question_id)).any():
                # Update existing entry
                idx = df.index[df["question_id"].astype(str) == str(question_id)].tolist()[0]
                df.at[idx, "rating"] = rating
            else:
                # Get the question from session state if available
                question_text = st.session_state.get("current_question", "No question text available")
                # Create a new entry with the question_id and rating
                new_row = {
                    "timestamp": timestamp,
                    "file_name": uploaded_file_name or 'None',
        ...
        # Only save to S3 when we have both a question and rating
...
def auto_save_unrated_questions():
    """
    Function to handle auto-saving unrated questions.
    Call this at the end of your Streamlit app.
    """
    # Check if we have a question that hasn't been rated but was answered
    if (st.session_state.get('current_question_id') and 
        not st.session_state.get('has_rated', False) and 
        st.session_state.get('current_question')):
        
        # If the session is ending without a rating, log with 'No Rating'
        if 'on_session_end' in st.session_state:
            ...
            st.session_state.on_session_end = False
...
def simple_finance_chat():
    ...
    if 'show_rating_reminder' not in st.session_state:
        st.session_state.show_rating_reminder = False
    ...
    try:
        ...
        if question and question != st.session_state.current_question:
            ...
            # Generate a question_id without logging yet
            question_id = log_question_and_rating_to_s3(question=question)
            st.session_state.current_question_id = question_id
            st.session_state.show_rating_reminder = False
            print(f"Set current_question_id in session state: {question_id}")
            
            # [Your Claude API call code and query execution remains unchanged]
            
            # After displaying the interpretation
            interpretation_section = st.container()
            with interpretation_section:
                # [Result interpretation code remains unchanged]
                
                st.write("### How would you rate this answer?")
                
                # Create a form for rating submission
                with st.form(key="rating_form"):
                    st.write("Select your rating:")
                ...
                def submit_rating(rating_value):
                    ...
                    # Now log both question and rating in one operation
                    if question_id:
                        # Log with existing question_id
                        log_question_and_rating_to_s3(
                            question_id=question_id, 
                            rating=rating_str, 
                            uploaded_file_name="TestDoc"
                        ...
                        # Fallback if no question_id is available
                        log_question_and_rating_to_s3(
                            question=st.session_state.current_question, 
                            rating=rating_str, 
                            uploaded_file_name="TestDoc"
                ...
                # Add a reminder to rate if user tries to ask another question without rating
                if not st.session_state.has_rated and st.session_state.show_rating_reminder:
                    st.warning("Don't forget to rate this answer before asking a new question!")
        
        # Check if a new question is being typed in the input field
        elif st.session_state.current_question and not st.session_state.has_rated and question != st.session_state.current_question:
            # Set reminder flag if user changes to a new question without rating
            st.session_state.show_rating_reminder = True
            
    except Exception as e:
        st.error(f"Error loading data from S3: {str(e)}")
        
    # Add this at the end of your simple_finance_chat function
    # Setup session end callback
    if 'on_session_end' not in st.session_state:
        st.session_state.on_session_end = True
        
    # Call auto-save function at the end
    auto_save_unrated_questions()
