...
def get_dataframe_schema(df):
    ...
    # Get all unique metric values if metrics column is identified
    unique_metrics = []
    if 'metrics' in df.columns:
        unique_metrics = df['metrics'].dropna().unique().tolist()
    
    return schema_info, potential_client_columns, potential_metric_columns, unique_clients, unique_metrics

def format_schema_for_prompt(schema_info, potential_client_columns, potential_metric_columns, unique_clients, unique_metrics):
    """Format schema information for the LLM prompt with column classifications, unique clients and metrics"""
    schema_text = "DataFrame Schema:\n"
    ...
    # Add all unique metric values
    if unique_metrics:
        schema_text += "AVAILABLE METRICS (EXACT STRINGS FOR FILTERING):\n"
        for idx, metric in enumerate(unique_metrics, 1):
            schema_text += f"{idx}. '{metric}'\n"
        schema_text += "\n"
...
def answer_question_with_sql(df, question):
    """Main function to answer questions using SQL on a dataframe with improved client and metric matching"""
    # Get schema info with unique clients and metrics
    schema_info, client_cols, metric_cols, unique_clients, unique_metrics = get_dataframe_schema(df)
    schema_text = format_schema_for_prompt(schema_info, client_cols, metric_cols, unique_clients, unique_metrics)
    
    # Generate SQL with improved client and metric matching
    sql_query = generate_sql_query(question, schema_text, unique_clients, unique_metrics)
